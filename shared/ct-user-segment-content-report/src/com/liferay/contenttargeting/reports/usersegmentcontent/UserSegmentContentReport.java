...
public class UserSegmentContentReport extends BaseReport {
	...
	protected void populateContext(Map<String, Object> context) {
		long userSegmentId = MapUtil.getLong(context, "userSegmentId", 0);

		List<UserSegmentContent> userSegmentContents = Collections.emptyList();

		int totalUserSegmentContents = 0;

		if (userSegmentId == 0) {
			context.put("userSegmentContents", userSegmentContents);
			context.put("totalUserSegmentContents", totalUserSegmentContents);
		...
		try {
			userSegmentContents =
				UserSegmentContentLocalServiceUtil.getUserSegmentContents(
					userSegmentId, startAndEnd[0], startAndEnd[1],
					new UserSegmentContentCountComparator());

			totalUserSegmentContents =
				UserSegmentContentLocalServiceUtil.
					getUserSegmentContentsCount(userSegmentId);
		}
		catch (Exception e) {
			_log.error(
				"Cannot render report for user segment " + userSegmentId);
		}
